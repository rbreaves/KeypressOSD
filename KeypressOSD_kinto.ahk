...
GetKeyStr() {
	static modifiers := ["RCtrl", "LCtrl", "Shift", "Alt", "LWin", "RWin"]
	...
	if (key ~= "i)^(LCtrl|RCtrl|Shift|Alt|LWin|RWin)$") {
		if !ShowSingleModifierKey {
			throw
		}
		key := ""
		; if RTrim(prefix, "+ ") = "Ctrl" {
		; 	prefix = "Cmd"
		; }
		if InStr(prefix, "LCtrl"){
			prefix := strReplace(prefix, "LCtrl", "Cmd")
		}
		if InStr(prefix, "LWin"){
			prefix := strReplace(prefix, "LWin", "LCtrl")
		}
		prefix := RTrim(prefix, "+ ")
...
ReadSettings() {
	...
	IniRead, Bottom_OffsetX       , %IniFile%, Settings, Bottom_OffsetX       , -500
	IniRead, Bottom_OffsetY       , %IniFile%, Settings, Bottom_OffsetY       , 200
